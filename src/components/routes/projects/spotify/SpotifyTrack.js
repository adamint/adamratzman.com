...
export function SpotifyTrack({ track, openInNewTab = false, ...rest }) {
  const artistsComponent = reduceComponentsToString(track.artists.map(artist => <ChakraRouterLink
    to={`/projects/spotify/artists/${artist.id}`} key={artist.id} target={openInNewTab ? '_blank' : '_self'}>{artist.name}</ChakraRouterLink>), ', ');

  function TrackLink({ children }) {
